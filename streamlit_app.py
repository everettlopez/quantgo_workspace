# streamlit_app.py — QSW v2.7 (heartbeat + auto Cloudflare R2 upload)
...
st.set_page_config(page_title="QSW — Quantum Go/No-Go", layout="wide")
...
def save_bar(df, col, title, ylabel, path: Path):
    fig, ax = plt.subplots()
...
def r2_upload_dir(local_dir: Path, bucket: str, prefix: str):
    ...
    for p in local_dir.rglob("*"):
        ...
        try:
            s3.upload_file(str(p), bucket, key, ExtraArgs=extra)  # private by default
            uploaded += 1
        except Exception as e:
            skipped.append((rel, f"{type(e).__name__}: {e}"))
    return uploaded, skipped

# -------------------- load project modules (soft) --------------------
has_modules = True
try:
    import sys
    ...
    has_modules = False
    st.error(f"Import error — ensure qsw_go_nogo_v2/ is present: {e}")

# -------------------- UI --------------------
st.title("Quantum Strategy Workbench — Go/No-Go")

st.sidebar.header("Providers & Purity")
prov_choices = st.sidebar.multiselect(
    "Quantum providers (priority order)",
    options=["anu","json"],  # if you configured your JSON provider in qrng_client_v2
    default=["anu"]
)
...
def run_condition(n_trials, condition, seed, phase_len, max_steps,
    ...
    for i in range(1, n_trials + 1):
        ...
        if save_trials:
            trial_rows.append({
                "condition": condition, "trial_idx": i,
...
if start_disabled:
    st.warning("Missing modules in qsw_go_nogo_v2/. Add project files and rerun.")
...
if st.button("Start Run", disabled=start_disabled):
    ...
    if not conditions:
        ...
        st.write("### Run Summary"); st.dataframe(df, use_container_width=True)

        # advisory on QRNG health
        ...
        p4 = out_dir / "mi.png"
        def save_plot(col, title, ylabel, path):
            save_bar(df, col, title, ylabel, path)
        save_plot("success_rate", "Success Rate — Impasse Escape (v2.7)", "Success Rate", p1)
        save_plot("avg_time_to_escape", "Avg Time to Escape (lower=better)", "Avg Time to Escape", p2)
        save_plot("diversity_bits", "Diversity (bits)", "Diversity (bits)", p3)
        save_plot("early_gate_success_MI", "Early Gate ↔ Success MI (nats)", "MI (nats)", p4)
        ...
        meta = {
            ...
            "params": {"trials": int(trials), "phase_len": int(phase_len),
        ...
        for rp in [out_dir / "raw" / "raw_trials.csv.gz", out_dir / "raw" / "raw_gates.csv.gz"]:
            if rp.exists(): manifest[rp.name] = {"sha256": _file_sha256(rp), "bytes": rp.stat().st_size}
